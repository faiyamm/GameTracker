...
struct AddEditView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section(header: Text("Basic Info").foregroundColor(.blue).bold()) {
                ...
                Section(header: Text("Description").foregroundColor(.blue).bold()) {
                    TextEditor(text: $workingGame.description)
                        .frame(height: 100)
                }
                
                Section(header: Text("Rating & Review").foregroundColor(.blue).bold()) {
                    StarRatingView(rating: $workingGame.rating)
                    TextEditor(text: $workingGame.review)
                        .frame(height: 100)
                }
            }
            .navigationTitle(game.title.isEmpty ? "New Entry" : "Update Entry")
            .toolbar {
